...
与 parity suite 分离的最新不可变 Anera 内部能力证据为 `reports/quality-benchmarks/harness-quality-2026-08-30T09-44-00-127Z.json`（schema `anera-harness-quality/1.9`）：18/18 desktop-only 自然/对抗任务通过、平均质量 99.44，critical 与 efficiency 全通过，115 model/118 tool，`failedToolCalls:0`；五个 trajectory-blind Judge 均为 10/10。唯一 90 分是旧 evaluator 未识别 Artifact 中 `Exclusions` 名词形式的假阴性；oracle 修复后的独立数据分析复跑 `reports/quality-benchmarks/harness-quality-2026-08-30T09-45-37-971Z.json` 为 100，历史完整报告不回写。该报告明确写入 `arenaParityEvidence:false` 与 `mobileExcluded:true`；它证明候选 Harness 在冻结任务上的完成能力，既不填充 65 题 manifest 的 Arena reference，也不能把 `visualBaselinePassed` 或 observable parity 状态改为 true。

生产研究链新增的 `model.final.repair` 与既有 `model.tool_call.repair`、`provider.usage` 一样属于 Anera 内部质量/计量事件，normalizer 明确排除，不能污染 Arena observable event alignment。确定性回归证明缺少 exact evidence URL 或引用 unsupported URL 的 research draft 不会成为 Final，最多一次纠正后只发布 grounded Final；真实 `reports/real-smokes/active-research-2026-08-30T10-05-17.356Z.json` 则证明无需纠正的正常路径仍保持 canonical `search×1/fetch×2`。
