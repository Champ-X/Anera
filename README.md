...
上下文采用持久证据与模型输入分离：已消费的大段工具文本按需投影为摘录，原文保留于会话，并以 session-local SHA-256 记录供 `read_context` 分页/搜索取回。新结果、失败/未知执行结果及图像不做此投影；检索不等于重新验证，也不放宽修改与交付门禁。摘要尾部按消息大小留出空间，同时保留强制证据锚点及最新工具配对。单轮模型请求数和累计 token 默认均不设硬上限；可通过 `ANERA_MAX_AGENT_MODEL_REQUESTS_PER_TURN` 和 `ANERA_MAX_AGENT_TOTAL_TOKENS_PER_TURN` 设置正整数限制，`0` 表示关闭。物理请求的持久预留与用量计量始终保留；上下文窗口、超时、取消和无进展保护继续生效。

浏览器操作快照自动附带当前页面的运行错误，截图结果在存在错误时也附带诊断；每次 Agent 决策补充浏览器已捕获的异步错误。诊断按文档导航隔离，普通日志不会挤掉错误，旧页面的异常不会污染重新打开后的结果。诊断只证明已观察到的问题，不把无错误等同于验证通过。相同 Bash 命令连续执行失败时，即使中间修改了文件，也会基于持久事件提示重新检查排查策略；成功执行会重置该提示，提示本身不终止任务。
